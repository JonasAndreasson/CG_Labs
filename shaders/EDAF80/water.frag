...
vec3 ripples(in vec3 texCoord)
{
	...
	vec4 n0 = normalize(2*texture(wave_texture, normalCoord0)-vec4(1.0));
	vec4 n1 = normalize(2*texture(wave_texture, normalCoord1)-vec4(1.0));
	vec4 n2 = normalize(2*texture(wave_texture, normalCoord2)-vec4(1.0));
...
void main()
{
	...
	vec3 n = ripples(vec3(fs_in.texture, 0.0));
	...
	float eta = 1.0/1.33;
	vec4 refraction = vec4(texture(cubemap,	refract(V, n, eta)));
	float fresnel = fresnel_calc(0.02037, V, n);
	frag_color = reflection;//water_color + reflection;//*fresnel +refraction * (1-fresnel);
}
